...
import (
	...
	"github.com/sadath-12/keywave/internal/grpcutil"
...
func (o Opts[T]) Distribute(ctx context.Context, mapFn MapFn[T], reduceFn ReduceFn[T]) error {
	...
	for {
		...
		case reply, ok := <-replies:
			...
			if reply.err == nil {
				// since we added sync for all nodes we send req to all nodes to put values and if we recieve max of them we return bt back of the play after this also
				// they all will perform operation
